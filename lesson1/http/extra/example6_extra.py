...
issues_url = get_url('api/issuses')
...
issue_data = {'name': 'bypython',
              'due_data': '2009-02-11',
   	      'description': 'we have'
     	     }

# create issue (success: withoutdata)
response = requests.post(issues_url, data=json.dumps(issue_data), headers=headers, cookies=cookies)
...
response = requests.path(issue_url(created_issue['id']), data=json.dumps(new_data), headers=headers, cookies=cookies)
print(response.status_code)
print(response.json())
...
# SESSION EXAMPLE
# auth and get issues
data = {'username': 'testusers', 'password': 'pass'}
headers = {'Content-Type': 'Applications/json'}
...
# pool manager
adapter = HTTPAdapter(pool_maxsize=10,pull_block=True,pull_connections=10)
session = requests.Session()
